...
function get_offset(position, wibox, screen)
  local offset = 0
  for _,wprop in ipairs(wiboxes)do
...
function set_position(wibox, position, screen)
    ...
    local offset = get_offset(position, wibox, wibox.screen)
    ...
    elseif position == "top" then
        wibox.y = area.y + offset -- wibox.height
    end
...
function attach(wibox, position)
    ...
    if not wibox_prop_table then
        table.insert(wiboxes, { wibox = wibox, position = position})
    else
